...
from motion_planning.utils import Shelf
...
class MuJoCoRenderer:
    ...
    def _setup_scene(self):
        ...
        for obs in self.config.get('obstacles', []):
            obstacles.append(Obstacle(center=np.array(obs['center']),
                                    radius=obs['radius'],
                                    safe_distance=obs['safe_distance']))

        target_position = np.array(self.config['target_position'])

        # If shelf parameters are present, build shelf scene; else fall back.
        shelf_keys = ('shelf_origin', 'shelf_depth', 'shelf_height', 'bay_widths')
        print("[GIF] config shelf keys:",
                self.config.get('shelf_origin'),
                self.config.get('shelf_depth'),
                self.config.get('shelf_height'),
                self.config.get('bay_widths'))
        if all(k in self.config and self.config[k] is not None for k in shelf_keys):
            shelf = Shelf(origin=tuple(self.config['shelf_origin']),
                        depth=float(self.config['shelf_depth']),
                        height=float(self.config['shelf_height']),
                        bay_widths=tuple(self.config['bay_widths']),
                        collidable=True)
            scene_path = create_scene_with_elements(
                obstacles=obstacles,
                target_position=target_position,
                max_traj=1000,              # we render up to 1000 dots
                filename="scene_shelf.xml",
                shelves=[shelf],
                show_target=False
            )
        else:
            # fallback for non-shelf runs
            from scene_builder import create_standard_scene
        ...
        # --- after scene_path is set ---
        print("[GIF] using shelf builder:", all(k in self.config and self.config[k] is not None
                                            for k in ('shelf_origin','shelf_depth','shelf_height','bay_widths')))
        print("[GIF] scene path:", scene_path)
        try:
            with open(scene_path, "r") as f:
                xml_txt = f.read()
            print("[GIF] found 'shelf_' in XML:", ("shelf_" in xml_txt))
        except Exception as e:
            print("[GIF] could not read scene xml:", e)
