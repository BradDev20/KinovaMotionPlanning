...
class MujocoSceneBuilder:
    ...
    def add_shelf(self, shelf: Shelf) -> Tuple[dict, Tuple[float, float], Tuple[float, float]]:
        """
        Append a Shelf's XML and return (shelf_box, bayA, bayB).
        shelf_box: {'x': (x0,x1), 'y': (y0,y1), 'z': (z0,z1)}
        """
        self.xml_elements.append(shelf.to_xml())
        shelf_box = {'x': shelf.x_bounds(), 'y': shelf.y_bounds(), 'z': shelf.z_bounds()}
        bayA, bayB = shelf.bay_intervals()
        return shelf_box, bayA, bayB

    def add_shelves(self, shelves: List[Shelf]) -> 'MujocoSceneBuilder':
        for sh in shelves:
            self.xml_elements.append(sh.to_xml())
        return self
    ...
    def add_target(self, position: np.ndarray, size: float = 0.03, visible: bool = True) -> 'MujocoSceneBuilder':
        if visible:
            target_xml = f'''
        <body name="target_marker" pos="{position[0]} {position[1]} {position[2]}">
            ...
            <site name="target_center" pos="0 0 0" size="0.005" rgba="0 0 0 0"/>
        </body>'''
        else:
            # Site only (no visible geom)
            target_xml = f'''
        <body name="target_marker" pos="{position[0]} {position[1]} {position[2]}">
            <site name="target_center" pos="0 0 0" size="0.001" rgba="0 0 0 0"/>
        </body>'''
...
def create_scene_with_elements(
    ...
    builder = MujocoSceneBuilder(base_scene_path='robot_models/kinova_gen3/scene.xml')
    if shelves: builder.add_shelves(shelves)
    builder.add_obstacles(obstacles)
    if show_target:
        builder.add_target(target_position, visible=True)
    else:
        # Keep a site (invisible) so you still have a named reference if you ever need it
        builder.add_target(target_position, visible=False)
    builder.add_trace_dots(max_traj * 10, default_color="1 1 1 0.8")
    return builder.build_scene(filename)
